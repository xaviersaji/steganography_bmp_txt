...
void dbit_tolsb_char(encodeInfo *encinfo, int size, int p)
{
    int mask,i,f=0;
    char im;

    fseek(encinfo->fp_src_image,(p), SEEK_SET);
    // assign each bit to lsb of image
    for(i=0; i<size; i++)
    {
        ...
        // make sure the cursor is at appropriate place
        fseek(encinfo->fp_steg,(p+i), SEEK_SET);
...
int extract_secret(encodeInfo *encinfo)
{
  ...
  printf("%d\n",encinfo->size_secret);
    ...
    {
        dbit_tolsb_char(encinfo, 8, (166+i));
    }
